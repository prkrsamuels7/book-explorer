  ...
  try {
    for (const genre of genres) {
      const response = await fetch(
        `https://www.googleapis.com/books/v1/volumes?q=subject:${genre}&maxResults=20&key=${API_KEY}`
      ); // Returns 40 books each genre
    ...
    for (const book of books) {
      ...
      newCategories = newCategories.reduce(
        (acc, cat) => acc.concat(cat.split(" / ")),
        []
      );
